...
class Qwen2_5_VLForConditionalGenerationForJanusVLN(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # Optional SAM3 integration (disabled by default). When enabled, we lazily
        # load the SAM3 image predictor and keep a light projection head to turn
        # box+score features into object tokens. This keeps the default inference
        # path intact if the user does not provide a checkpoint.
        self.use_sam3 = getattr(config, "use_sam3", False)
        self.sam3_checkpoint = getattr(config, "sam3_checkpoint", None)
        self.sam3_processor = None
        self.sam3_obj_proj = None
        self.last_sam3_obj_tokens = None
        if self.use_sam3 and self.sam3_checkpoint is not None:
            try:
                from sam3.model_builder import build_sam3_image_model  # type: ignore
                from sam3.model.sam3_image_processor import Sam3Processor  # type: ignore

                sam3_model = build_sam3_image_model(checkpoint_path=self.sam3_checkpoint)
                self.sam3_processor = Sam3Processor(sam3_model)
                # Project [x1,y1,x2,y2,score] to hidden size as a placeholder object token.
                self.sam3_obj_proj = nn.Linear(5, config.hidden_size)
            except Exception as e:  # pragma: no cover - optional path
                warnings.warn(f"Failed to load SAM3 (use_sam3={self.use_sam3}): {e}")
                self.use_sam3 = False
    ...
    def _run_sam3(self, pil_image):
        """
        Run SAM3 on a single PIL image to obtain lightweight object tokens.
        Returns a tensor [num_obj, hidden_size] or None if disabled/failed.
        """
        if not (self.use_sam3 and self.sam3_processor is not None and self.sam3_obj_proj is not None):
            print("[SAM3] skip: use_sam3 is False or SAM3 not initialized.")
            return None
        try:
            state = self.sam3_processor.set_image(pil_image)
            out = self.sam3_processor.set_text_prompt(state=state, prompt="beds")
            ...
            box_t = torch.tensor(
                boxes,
                device=self.sam3_obj_proj.weight.device,
                dtype=self.sam3_obj_proj.weight.dtype,
            )
            box_t_norm = box_t.clone()
            box_t_norm[:, [0, 2]] = box_t_norm[:, [0, 2]] / max(w, 1)
            box_t_norm[:, [1, 3]] = box_t_norm[:, [1, 3]] / max(h, 1)
            score_t = torch.tensor(
                scores, device=box_t_norm.device, dtype=box_t_norm.dtype
            ).unsqueeze(1)
            feats = torch.cat([box_t_norm, score_t], dim=1)  # [N,5]
            obj_tokens = self.sam3_obj_proj(feats)  # [N, hidden]
            # Debug print for downstream inspection
            preview = obj_tokens.detach().cpu()
            preview_str = (
                preview.flatten()[:8].tolist() if preview.numel() > 0 else []
            )
            print(
                f"[SAM3] detected {len(scores)} objects, obj_ptr shape={tuple(obj_tokens.shape)}, preview={preview_str}"
            )
            return obj_tokens
        except Exception as e:  # pragma: no cover - best-effort path
            warnings.warn(f"SAM3 inference failed, falling back: {e}")
            return None
